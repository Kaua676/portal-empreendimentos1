document.getElementById('loginForm').addEventListener('submit', function (event) {
    ...
    if (!validarSenha(senha)) {
        showToast('Senha deve conter no mínimo 8 caracteres, uma letra maiúscula, um número e um caractere especial.', 5000);
        return;
    }
    ...
    fetch('api/loginControllerProxy.php', {
        method: 'POST',
        body: formData,
    })
        ...
        .then(data => {
            if (data.status === 'blocked') {
                ...
                showToast(`Você foi bloqueado por ${data.tempoRestante} segundos.`, 6000);
                ...
                setTimeout(() => window.location.href = 'forcePasswordChange.php', 1500);
            } else if (data.status === 'success') {
                showToast('Login bem-sucedido! Enviando código de verificação...', 2000);
                ...
                fetch('api/twoFactorStartProxy.php')
                    ...
                    .then(({ status, message }) => {
                        if (status === 'success') {
                            ...
                            setTimeout(() => {
                                window.location.href = 'twoFactorVerify.php';
                            }, 1500);
                        } else {
